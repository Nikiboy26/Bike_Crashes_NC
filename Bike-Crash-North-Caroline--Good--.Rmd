---
title: "Bike Crash North Caroline"
...
The dataset has a lot of columns. Some columns are not interesting for particular question, some columns have too many NA's. OTher columns like alcohol were removed because number of drivers who test positive for alcohol almost non-existent. 
...
```{r}
summary(as.factor(bikes$bikeagegrp))
```

Looks like a mistake during the data entry stage and some numbers became month names. 
...
#### Basic EDA 
I decided to go with 'Ambulancer' column as an indicator of the severity of an accident. There are some other columns, but they contain categories like 'suspected" or 'possible injury'. Here I assume that if there is an ambulance, the case is serious enough to investigate the reasons. 
...
bikes%>%
  ...
  count()%>%
  ggplot(aes(x = crashyear, y = n, fill = ambulancer)) + geom_col() + ylim(0, 800) +
  scale_x_continuous(breaks = c(2007:2019)) + labs(x = 'Year',
                                                   y = 'Number of crashes',
                                                   title = 'Bike crashes every year') + 
  scale_fill_manual(values=c("#2CD3F8", "#F6685F")) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
                                                            panel.background = element_blank(), axis.line = element_line(colour = "black"))
  
```

From looking at number of crashes per year I can't say that we see rise increase in traffic accidents
...
bikes%>%
  count(crashmonth)%>%
  ggplot(aes(x = crashmonth, y = n)) + geom_col() + labs(x = 'Month',
                                                         y = 'Number of Crashes',
                                                         title = 'Crashes per Month') + 
  ylim(0, 1500) 
```

Monthly trend resembles average yearly temperature for North Carolina. This makes sense as warmer temperature means more people using bikes. 


```{r echo = F}
#Hourly difference 
bikes%>%
  group_by(crashhour, ambulancer)%>%
  count()%>%
  ggplot(aes(x = crashhour, y = n, fill = ambulancer)) + geom_col() + 
  scale_fill_manual(values=c("#2CD3F8", "#F6685F")) + labs(x = 'Hour',
                                                           y = 'Number of Crashes', 
...
#Age groups 
bikes%>%
  group_by(bikeagegrp, ambulancer)%>%
  count()%>%
  ggplot(aes(x = bikeagegrp, y = n, fill = ambulancer)) + geom_col() + ylim(0, 2000) + 
  scale_fill_manual(values=c("#2CD3F8", "#F6685F")) + labs(x = 'Age Group',
                                                           y = 'Number of Crashes', 
                                                           title = 'Number of crashes per Age Group') 
```

Looks like in 25-29 group people are less likely to get into accidents and then chances get higher as one gets older 


```{r echo = F}
#Directions 
bikes%>%
  group_by(bikedir, ambulancer)%>%
  count()%>%
  ggplot(aes(x = reorder(bikedir, n), y = n, fill = ambulancer)) + geom_col() + 
...
Most crashes happen while bikers ride along the traffic. This could mean poorly developed biking paths system which is not surprising when we talk about US cities.
...
bikes%>%
  group_by(city, ambulancer)%>%
  count()%>%
  arrange(desc(n))%>%
  head(20)%>%
  ggplot(aes(x = reorder(city,n), y = n, fill = ambulancer)) + geom_col() + labs(x = 'Number of Crashes',
                                                              y = 'City',
                                                              title = 'Cities with most crashes') + 
  scale_fill_manual(values=c("#2CD3F8", "#F6685F")) + 
  coord_flip() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
                                                            panel.background = element_blank(), axis.line = element_line(colour = "black"))
```

Here we can see that more populous cities have more crashes which can be explained by the amount of people using bikes there. But most of crashes actually happen in rural areas that lack bike infrastructure. This might be another indicator that fixing biking infrastructure can actually increase the situation. 
...
```{r}
bikes$ambulancer <- as.factor(bikes$ambulancer)
...
vip::vip(model) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
                                                            panel.background = element_blank(), axis.line = element_line(colour = "black"))
```

Feature importance hepls us to identify features that are heaving most impact on outcome variable. Even though it's a good indicator for the analysis we have to be careful while interpreting these results. More domain knowledge is needed. 
Here we see that the time when a crash happened has most affect on a severity of an accident. 
...
predictions_combined <- predictions %>% 
  mutate(true_class = bikes_test$ambulancer)
...
Current accuracy is 0.7116345, which can be improved by tuning and using boosted-trees. But I would not be expecting big increase in model performance since we are trying to predict real-life events involving humans.